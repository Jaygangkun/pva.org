...
class BreadCrumbFactory {
    ...
    private function get_home() {
        if ( is_front_page() ) {                   
            ...
            if($post_type == 'news' || ($this->term_object && $this->term_object->slug == 'recent-news'))
            {
                $media_link = home_url('/news-and-media-center/');
                $media_name = __( 'News And Media Center' );

                $this->breadcrumbs[] = new Breadcrumb($media_link,$media_name);
            }

            if($post_type == 'member')
            {
                $hero_stories_link = home_url('/about-us/our-members/');
                $hero_stories_name = __( 'Hero Stories' );

                $this->breadcrumbs[] = new Breadcrumb($hero_stories_link,$hero_stories_name);
            }

            if(isset($_GET['s']) && isset($_GET['post_type']) && !empty($_GET['s']) && $_GET['post_type'] == 'member')
            {
                $about_link = home_url('/about-us/');
                $about_name = __( 'About Us' );

                $this->breadcrumbs[] = new Breadcrumb($about_link,$about_name);

                $hero_stories_link = home_url('/about-us/our-members/');
                $hero_stories_name = __( 'Our Members' );

                $this->breadcrumbs[] = new Breadcrumb($hero_stories_link,$hero_stories_name);

                $this->breadcrumbs[] = new Breadcrumb(false,'Member Search');
            }
        }
    }
